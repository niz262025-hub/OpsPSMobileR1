...
export default function TripDetail() {
  ...
  const handleCloseTrip = async () => {
    ...
    try {
      await updateDoc(doc(db, 'trips', tripId), { status: 'closed' });
      Alert.alert('Trip closed', 'The trip has been closed successfully.');
      router.push('/(tabs)/inventory');
    } catch (error) {
      console.error('Close trip error:', error);
      Alert.alert('Error', 'Unable to close this trip.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Pressable style={styles.primaryButton} onPress={() => router.push({ pathname: '/upload-product', params: { tripId: trip.id } })}>
          <Text style={styles.primaryText}>Upload Product</Text>
        </Pressable>

        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>Trip Workflow</Text>
          <Text style={styles.infoText}>Open Trip → Upload Products → Marketplace → Customer Orders → Close Trip → Transfer to Inventory → Orders become Ready to Pack.</Text>
        </View>

        <Pressable style={styles.closeButton} onPress={handleCloseTrip}>
          <Text style={styles.closeText}>Close Trip & Transfer to Inventory</Text>
        </Pressable>
