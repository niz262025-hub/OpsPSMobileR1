...
export default function FounderRegistrationScreen() {
  ...
  return <RegistrationLayout title="Register Founder" onBack={() => router.back()}><Field label="Founder Name" value={form.name} onChangeText={(value) => update('name', value)} /><Field label="Business Name" value={form.businessName} onChangeText={(value) => update('businessName', value)} /><Field label="Email / Username" value={form.email} onChangeText={(value) => update('email', value)} /><Field label="Phone" value={form.phone} onChangeText={(value) => update('phone', value)} /><Field label="Business Address" value={form.address} onChangeText={(value) => update('address', value)} /><Field label="Password" value={form.password} onChangeText={(value) => update('password', value)} secureTextEntry /><Field label="Confirm Password" value={form.confirmPassword} onChangeText={(value) => update('confirmPassword', value)} secureTextEntry />{!!error && <Text style={styles.error}>{error}</Text>}<Pressable style={styles.primary} onPress={submit}><Text style={styles.primaryText}>Register Founder</Text></Pressable></RegistrationLayout>;
}

function RegistrationLayout({ title, onBack, children }: { title: string; onBack: () => void; children: React.ReactNode }) { return <SafeAreaView style={styles.safe}><View style={styles.container}><Pressable onPress={onBack} style={styles.back}><ArrowLeft size={18} color="#5B2BD9" /><Text style={styles.backText}>Back</Text></Pressable><Text style={styles.brand}>Ops<Text style={styles.accent}>PS</Text></Text><View style={styles.form}><Text style={styles.title}>{title}</Text>{children}</View></View></SafeAreaView>; }
