    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        <ScrollView
          ...
          bounces={false}
        >
          <Pressable onPress={() => router.back()} style={styles.back}><ArrowLeft size={18} color="#5B2BD9" /><Text style={styles.backText}>Back</Text></Pressable>
          <Text style={styles.brand}>Ops<Text style={styles.accent}>PS</Text></Text>
          <View style={styles.form}>
            <Text style={styles.title}>Register Customer</Text>
            {[['name', 'Full Name'], ['email', 'Email / Username'], ['phone', 'Phone'], ['address', 'Address'], ['password', 'Password'], ['confirmPassword', 'Confirm Password']].map(([key, label]) => <View style={styles.field} key={key}><Text style={styles.label}>{label}</Text><TextInput value={form[key as keyof typeof form]} onChangeText={(value) => update(key as keyof typeof form, value)} secureTextEntry={key.includes('password')} autoCapitalize="none" style={styles.input} /></View>)}
            {!!error && <Text style={styles.error}>{error}</Text>}
            <Pressable style={styles.primary} onPress={submit}><Text style={styles.primaryText}>Register Customer</Text></Pressable>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
